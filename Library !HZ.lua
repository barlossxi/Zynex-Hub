...
do
	...
	function o:SetTheme(_)
		local a = p.Presets[_]
		if not a then
			p:Log("Theme not found: " .. tostring(_))
			return false
		end
		p.Themes = a
...
function p:Window(_)
	...
	local d = p:Create("TextLabel", {
		...
		Position = UDim2.new(0, m.TabWidth + 26, 0, 56),
		BackgroundColor3 = Color3.fromRGB(255, 255, 255),
		BackgroundTransparency = 1,
		BorderColor3 = Color3.fromRGB(0, 0, 0),
		Parent = n,
	...
	local f = p:Create("CanvasGroup", {
		...
		Parent = n,
		Visible = true,
		Position = UDim2.new(0, m.TabWidth + 26, 0, 94),
		Size = UDim2.new(1, -(m.TabWidth + 32), 1, -102),
		BackgroundColor3 = Color3.fromRGB(255, 255, 255),
		BackgroundTransparency = 1,
		BorderColor3 = Color3.fromRGB(0, 0, 0),
		BorderSizePixel = 0,
	})
	local function GetVisibleScrollFrame()
	...
	local function IsPointerOverContent(ScrollFrame)
		local Position = ScrollFrame.AbsolutePosition
	...
	local function ScrollVisibleContent(Amount)
		local ScrollFrame = GetVisibleScrollFrame()
		if not ScrollFrame or not ScrollFrame.ScrollingEnabled or not IsPointerOverContent(ScrollFrame) then
			return
		end
		local Maximum = math.max(0, ScrollFrame.AbsoluteCanvasSize.Y - ScrollFrame.AbsoluteWindowSize.Y)
		ScrollFrame.CanvasPosition = Vector2.new(
			ScrollFrame.CanvasPosition.X,
			math.clamp(ScrollFrame.CanvasPosition.Y + Amount, 0, Maximum)
		)
	end
	p:Connect(l.InputChanged, function(Input)
	...
	function k:AddTab(_)
		...
		local g = p:Create("ScrollingFrame", {
			...
			ScrollBarImageColor3 = p.Themes.BackgroundColor,
		...
		local function UpdateCanvasSize()
			g.CanvasSize = UDim2.fromOffset(0, ContentLayout.AbsoluteContentSize.Y + 2)
		end
		p:Connect(ContentLayout:GetPropertyChangedSignal("AbsoluteContentSize"), UpdateCanvasSize)
		UpdateCanvasSize()
		if b.Icon ~= "" then
		...
		function h:AddToggle(t)
			...
			function i:Set(val)
				...
				local Theme = p.Themes or p.Presets.Default
				local accent = o:LightenColor(Theme.BackgroundColor, 0.25)
